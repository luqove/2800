...
class DFRobot_TMF8x01:
    ...
    result_dictKey = ['status', 'regContents', 'tid', 'resultNumber', 'resultInfo', 'disL', 'disH', 'syscolck0',
                      'syscolck1', 'syscolck2', 'syscolck3']
    result_dict = {}
    ...
    def is_data_ready(self):
        ...
        if self.result_dict['regContents'] == 0x55:
            if self.result_dict['tid'] != self._tid:
                ...
                j = self.result_dict["syscolck3"] << 24 | self.result_dict["syscolck2"] << 16 | self.result_dict[
                    "syscolck2"] << 8 | self.result_dict["syscolck0"]
                if self._count < 4:
